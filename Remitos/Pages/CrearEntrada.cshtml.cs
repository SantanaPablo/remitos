...
public class CrearEntradaModel : PageModel
{
    ...
    [BindProperty]
    public NotaEntrada Nota { get; set; } = new();

    [BindProperty]
    public List<ItemEntrada> Items { get; set; } = new();

    public List<SelectListItem> Autorizantes { get; set; } = new();
    public List<string> Tecnicos { get; set; } = new();

    public CrearEntradaModel(AppDbContext context, IHttpContextAccessor httpContextAccessor)
    {
        _context = context;
    ...
    public void OnGet()
    {
        ...
        if (Items.Count == 0) Items.Add(new ItemEntrada());
    ...
    public IActionResult OnPostAddItem()
    {
        ...
        Items.Add(new ItemEntrada{ Unidad = "1" });
    ...
    public async Task<IActionResult> OnPostGuardarAsync()
    {
        ...
        {
            ModelState.AddModelError("Nota.Items", "Debe agregar al menos un ítem de entrada.");
        }
        ...
        return RedirectToPage("VerEntrada", new { id = Nota.Id });
    }
    ...
    {
        var json = _httpContextAccessor.HttpContext.Session.GetString("ItemsEntradaJson");
        if (!string.IsNullOrEmpty(json))
        {
            Items = JsonConvert.DeserializeObject<List<ItemEntrada>>(json);
        }
    }

    private void GuardarItemsEnSession()
    {
        _httpContextAccessor.HttpContext.Session.SetString("ItemsEntradaJson", JsonConvert.SerializeObject(Items));
    }

    private void LimpiarSession()
    {
        _httpContextAccessor.HttpContext.Session.Remove("ItemsEntradaJson");
